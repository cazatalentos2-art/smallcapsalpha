...
export default function WatchlistPage() {
  ...
  const [form, setForm] = useState({ ticker: '', name: '', notes: '', target_price: '', stop_loss: '' });
  ...
  const handleAdd = async () => {
    ...
    setForm({ ticker: '', name: '', notes: '', target_price: '', stop_loss: '' });
    setOpen(false);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="bg-card border-border">
            ...
            <div className="space-y-3 pt-2">
              ...
              <Button onClick={handleAdd} className="w-full text-xs">Guardar</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="grid gap-3">
        {items.length === 0 && !isLoading && (
          <div className="bg-card border border-border rounded-lg p-12 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Añade tickers desde el dashboard o el botón de arriba</p>
          </div>
        )}
        {items.map(item => (
          <div key={item.id} className="bg-card border border-border rounded-lg p-4 flex items-center justify-between hover:border-primary/30 transition-colors group">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <Link to={`/stock/${item.ticker}`} className="font-mono font-bold text-sm hover:text-primary transition-colors">
                    {item.ticker}
                  </Link>
