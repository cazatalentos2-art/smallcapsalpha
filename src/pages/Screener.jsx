      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {filtered.length} de {stocks.length} acciones
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          ...
          {filters.minRvol > 0 && (
            <Badge variant="outline" className="text-[10px] bg-accent/10 text-accent border-accent/30">
              RVOL ≥ {filters.minRvol}x
            </Badge>
          )}
          {filters.maxFloat < 100 && (
            <Badge variant="outline" className="text-[10px] bg-chart-5/10 text-chart-5 border-chart-5/30">
              Float ≤ {filters.maxFloat}M
            </Badge>
          )}
          {filters.minShortInterest > 0 && (
            <Badge variant="outline" className="text-[10px] bg-destructive/10 text-destructive border-destructive/30">
              SI ≥ {filters.minShortInterest}%
            </Badge>
          )}
        </div>
      </div>
